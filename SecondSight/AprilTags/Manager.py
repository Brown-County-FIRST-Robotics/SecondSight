...
class ApriltagManager:
    ...
    def fetchApriltags(self):
        ...
        year=SecondSight.config.Configuration().get_value('year', '2023')
        if len(cams) > 0:
            executor = concurrent.futures.ThreadPoolExecutor(max_workers=len(cams))
            futures = {}
            for i, cam in enumerate(cams):
                if cam.hasRole('apriltags'):
                    futures[i] = executor.submit(SecondSight.AprilTags.Detector.getCoords, cam.gray, year)
            for i, future in futures.items():
                dets = future.result()
                if len(dets) == 1:
                    a = SecondSight.AprilTags.Detector.getRelativePosition(dets[0], SecondSight.Cameras.CameraManager.getCamera(i).camera_matrix, None, year)
                    ...
                    a = SecondSight.AprilTags.Detector.getFieldPosition(dets, SecondSight.Cameras.CameraManager.getCamera(i).camera_matrix, None, year)
                    self.publishers[i][0].set([a.x, a.y, a.z, a.rotation.w, a.rotation.x, a.rotation.y, a.rotation.z], math.floor(SecondSight.Cameras.CameraManager.getTime(i) * 1000000))
